...
# except:
#     from Utility.File_Handler.ext.plyfile import PlyData, PlyElement
from Utility.File_Handler.ext.plyfile import PlyData, PlyElement
...
# TODO Does Meshlab / CloudCompare even use the 64 Bit information?
class PLYFileHandler:
    ...
    def __ply_data_vertices_to_vetex_list(ply_data):
        ...
        for point_index, line in enumerate(ply_data['vertex'].data):
            ...
            if use_color:
                current_point.color = np.array([line['red'], line['green'], line['blue']])
            ...
            if 'measurements' in line.dtype.names:
                ...
                for measurement_idx in range(len(line['measurements']) / elements_per_measurement):
                    array_idx = measurement_idx * elements_per_measurement
                    slice = line['measurements'][array_idx: array_idx + elements_per_measurement]
    ...
    @staticmethod
    def parse_ply_file_extended(path_to_file, indent_level=0):

        logger.info('Parse PLY File: ...')

        ply_data = PlyData.read(path_to_file)

        vertices, ply_data_vertex_dtype, ply_data_vertex_data_dtype = \
            PLYFileHandler.__ply_data_vertices_to_vetex_list(
            ply_data)
        faces, ply_data_face_type, ply_data_face_data_type = \
    ...
    @staticmethod
    def write_camera_ply_file(output_path_to_file, cameras, plain_text_output=True, indent_level=1):
        ...
        output_ply_data_vertex_element = PLYFileHandler.__cameras_2_ply_vertex_element(cameras, ply_data_vertex_data_dtype)
    ...
    @staticmethod
    def read_and_write_test(input_path_to_file, output_path_to_file):

        vertices, faces = PLYFileHandler.parse_ply_file(input_path_to_file)
        PLYFileHandler.write_ply_file(output_path_to_file, vertices, faces=faces)
...
if __name__ == "__main__":
    ...
    # ply_ofp = 'out.ply'
    #
    # measurements1 = [Measurement(1222.428, 2, 3, 4),
    #                  Measurement(8, 9, 10.1, 11.2),
    #                  Measurement(28, 29, 30, 31),
    #                  Measurement(12213, 29, 30, 31)]
    # measurements2 = [Measurement(11, 12, 13, 14),
    #                  Measurement(18, 19, 20, 21)]
    # p_1 = Point([0, 0, 0])
    ...
    # for p in vertices:
    #     print(p.coord, p.color, p.measurements)
